...
class EditorVisitor extends TemporaryVariableVisitor
{
    ...
    public function leaveNode(Node $node)
    {
        ...
        if ($this->isTemporaryVariableExpression($node) && $this->isTemporaryVariable($this->getVariableName($node->expr))) {
    ...
    {
        return $node->getType() == 'Expr_Variable' && key_exists($this->getActualFunction(), $this->variables) && key_exists($node->name, $this->variables[$this->getActualFunction()]);
    }
    ...
    {
        return key_exists($this->getActualFunction(), $this->temporaryVariables) && key_exists($name, $this->temporaryVariables[$this->getActualFunction()]) && $this->temporaryVariables[$this->getActualFunction()][$name];
    }
    private function isTemporaryVariableExpression(Node $node)
    {
        return $node->getType() == 'Stmt_Expression' && $node->expr->getType() == 'Expr_Assign' && $this->isTemporaryVariableAssign($node->expr);
    }
    private function isTemporaryVariableAssign(Assign $assign)
    {
        return $assign->var->getType() == 'Expr_Variable' && (in_array($assign->expr->getType(), ['Expr_ConstFetch', 'Expr_ClassConstFetch']) || $this->isFunctionCall($assign->expr));
    }
}